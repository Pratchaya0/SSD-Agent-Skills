...
user-invocable: true
argument-hint: "<command|partial> [target]"
allowed-tools:
  - AskUserQuestion
  - Read
  - Glob
...
พิมพ์ `{{command_prefix}}ssd <command>` เพื่อเรียกใช้คำสั่ง เช่น:
- `{{command_prefix}}ssd frontend-setup` — ตั้งค่าโปรเจค React/TypeScript
...
เมื่อผู้ใช้พิมพ์ `{{command_prefix}}ssd <command>`:
...
3. **พิมพ์ไม่ครบ/ไม่ตรงกับคำสั่งใดเลย (partial)** — แตกชื่อคำสั่งทั้งหมดด้วย `-` เป็น segment แล้วค้นหาคำสั่งที่มี segment ใด segment หนึ่ง "ขึ้นต้น" ด้วยข้อความที่พิมพ์ (prefix match ต่อ segment, ไม่สนตัวพิมพ์เล็ก/ใหญ่) ครอบคลุมทั้ง segment แรก (หมวด) และ segment ท้าย (keyword) เช่น:
   ...
   - **เจอ 1 คำสั่ง** — แนะนำคำสั่งนั้นทันที พร้อมคำอธิบายสั้นๆ และบรรทัด "ลองพิมพ์: `{{command_prefix}}ssd <command>`"
